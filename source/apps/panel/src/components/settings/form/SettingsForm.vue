<template>
  <div id="SettingsForm">
    ...
    />
    <SettingsLog :entries="logEntries" @click:delete-log="() => (logEntries = [])" />
  </div>
</template>
...
const values: Ref<SettingsTableRowValue[]> = ref(getSettingsTableValues(rows));

watch(values, () =>
  values.value.forEach((v: SettingsTableRowValue, i: number) => {
    if (v.state === SettingsTableRowState.Changed || v.state === SettingsTableRowState.Neutral)
...
function addLogMessage(message: string, type: LogEntryType = LogEntryType.Info) {
  ...
  try {
    ...
    if (response.status >= 200 && response.status <= 299) {
      values.value = values.value.map((v: SettingsTableRowValue, i: number) => {
        ...
        const row = rows[i];
        const engine = row.engine !== null ? engineData[row.engine - 100] : null;
        const thermal = row.thermal !== null ? thermalData[row.thermal - 100] : null;
        return {
          ...v,
          engine,
          thermal,
    ...
    if (err instanceof AxiosError) {
      ...
      addLogMessage("Parametry nemohly být načteny - neznámý error", LogEntryType.Error);
    }
  }
}

async function onSaveAction() {
  const changedValues: SettingsToSend[] = getChangedSettings(values.value, rows);
  if (changedValues.length === 0)
    return addLogMessage("Nebyly provedeny žádné změny, žádné nové parametry!", LogEntryType.Error);
  try {
    addLogMessage(`Odesílám změnu ${changedValues.length} parametrů`, LogEntryType.Info);
    ...
    onLoadAction();
    values.value = values.value = settingsSendToStatesError(changedValues, values.value, rows);
  }
}
