...
import { GitPullResult } from "../../domain/types/index.js";
...
function createCtx(
  ...
  return {
    ...
    fs: {
      ...
      copy: mock(async () => ok(undefined)),
      remove: mock(async () => ok(undefined)),
      rename: mock(async () => ok(undefined)),
      mkdir: mock(async () => ok(undefined)),
      readJson: mock(async () => ok({})),
      writeJson: mock(async () => ok(undefined)),
    },
    process: {
